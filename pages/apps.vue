...
<template>
  <div class="mx-auto max-w-7xl px-5 py-10 md:py-16">
    <section class="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
      <div><p class="mb-3 text-xs font-semibold uppercase tracking-[.24em] text-gold/80">Local services</p><h1 class="font-serif text-4xl font-semibold md:text-6xl">Apps</h1><p class="mt-4 max-w-2xl text-sm leading-7 text-white/45">Web apps currently listening on this Windows PC. Tailnet-ready services open using the same hostname you used for Inkrail.</p></div>
      <div class="flex gap-2"><button class="btn btn-quiet h-fit" :class="showAllEndpoints ? 'border-gold/50 text-gold' : ''" @click="showAllEndpoints = !showAllEndpoints">{{ showAllEndpoints ? 'Hide endpoints' : 'Show all endpoints' }}</button><button class="btn btn-quiet h-fit" :disabled="refreshing || pending" @click="rescan"><svg viewBox="0 0 24 24" class="h-4 w-4 fill-none stroke-current" stroke-width="2"><path d="M20 7v5h-5M4 17v-5h5"/><path d="M18.5 9a7 7 0 0 0-12-2M5.5 15a7 7 0 0 0 12 2"/></svg>{{ refreshing ? 'Scanning…' : 'Refresh' }}</button></div>
    </section>

    <div v-if="pending && !data" class="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3"><div v-for="index in 3" :key="index" class="surface h-44 animate-pulse rounded-2xl" /></div>
    ...
    <div v-else-if="visibleApps.length" class="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <article v-for="app in visibleApps" :key="`${app.protocol}-${app.port}`" class="surface group flex min-h-48 flex-col rounded-2xl p-5 transition hover:-translate-y-0.5 hover:border-gold/30">
        ...
        <div class="mt-5"><h2 class="truncate font-serif text-xl font-semibold" :title="app.name">{{ app.name }}</h2><p class="mt-1 text-xs text-white/35">{{ app.description }} · {{ app.protocol.toUpperCase() }} :{{ app.port }} · HTTP {{ app.status }}</p></div>
        <div class="mt-auto pt-5">
          <a v-if="address(app)" :href="address(app)" target="_blank" rel="noreferrer" class="btn btn-primary w-full">Open app ↗</a>
          <button v-else class="btn btn-quiet w-full cursor-not-allowed opacity-45" disabled>Open on the server PC</button>
        </div>
      </article>
    </div>
    <div v-else class="surface mt-10 rounded-2xl p-12 text-center"><h2 class="font-serif text-2xl font-semibold">No HTTP apps found</h2><p class="mt-3 text-sm text-white/40">Start a Web UI such as Plex or qBittorrent, then refresh this page.</p></div>

    <p v-if="data" class="mt-5 text-xs text-white/25">Scanned {{ data.listeners }} listening ports in {{ data.durationMs }} ms. Found {{ data.apps.length }} HTTP endpoints; {{ visibleApps.length }} shown.</p>
  </div>
</template>
