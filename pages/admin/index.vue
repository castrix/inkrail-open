...
const jobCategory = ref<'translation' | 'novel' | 'manga' | 'image'>(features.value?.translation ? 'translation' : 'novel')
...
const { data: activeJobData, refresh: refreshActiveJobs } = await useLazyFetch<any>('/api/jobs/active', { server: false, key: 'active-work' })
useVisiblePolling(async () => { await refreshJobs(); if (installedSources.value?.some(s => s.id === 'twkan')) await refreshBrowser() }, 10000)
...
<template>
  <div class="mx-auto max-w-6xl px-5 py-10 md:py-14">
    <div class="mb-10 flex flex-col justify-between gap-5 md:flex-row md:items-end">
      <div>
        <p class="mb-3 text-xs font-semibold uppercase tracking-[.24em] text-gold">Workspace</p>
        <h1 class="font-serif text-4xl font-semibold md:text-5xl">Collect and translate</h1>
        <p class="mt-3 text-sm text-muted">Downloads and translation jobs from your installed sources.</p>
      </div>
    ...
    <div v-if="blockedJobs.length" class="mb-8 rounded-2xl border border-[#d4a85f]/30 bg-[#d4a85f]/10 p-5 text-[#f0cb88]">
      <div class="font-semibold">Source access needs attention</div>
    ...
    <p v-if="!features?.translation" class="mt-5 rounded-xl border border-gold/30 p-4 text-sm">Translation is disabled. Downloads still work. Existing translation jobs wait until you set INKRAIL_ENABLE_TRANSLATION=true in tray Settings → Edit configuration, then apply configuration and restart.</p>
    <section class="mt-8">
      <div class="mb-4 flex items-end justify-between gap-4"><div><p class="text-xs uppercase tracking-[.18em] text-muted">Job history</p><h2 class="mt-1 font-serif text-3xl font-semibold">Workspace queue</h2></div><button class="text-sm text-gold" @click="() => refreshJobs()">Refresh</button></div>
      <div class="mb-4 flex gap-1 overflow-x-auto rounded-xl border border-white/10 bg-white/[.025] p-1">
        <button v-for="tab in jobTabs" :key="tab.id" class="flex shrink-0 items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold transition" :class="jobCategory === tab.id ? 'bg-white/10 text-white' : 'text-muted hover:text-white/70'" @click="selectJobCategory(tab.id)">
          {{ tab.label }} <span class="rounded-full bg-black/20 px-1.5 py-0.5 text-[9px]">{{ jobData?.counts?.[tab.id] || 0 }}</span>
        </button>
      </div>
      <div class="space-y-5">
        <div v-if="!jobs.length" class="surface rounded-2xl px-6 py-12 text-center text-sm text-muted">No jobs in this category yet.</div>
        <section v-for="section in jobSections" :key="section.id">
          <div class="mb-2 flex items-center gap-2 px-1"><span v-if="section.id === 'working'" class="h-2 w-2 animate-pulse rounded-full bg-gold" /><h3 class="text-xs font-semibold uppercase tracking-[.16em]" :class="section.id === 'working' ? 'text-gold' : 'text-muted'">{{ section.label }}</h3><span class="text-[10px] text-muted">{{ section.jobs.length }}</span></div>
          <div class="surface overflow-hidden rounded-2xl">
            <div v-for="job in section.jobs" :key="job.id" class="grid gap-3 border-b px-4 py-4 last:border-0 sm:px-5 md:grid-cols-[minmax(0,1fr)_auto_190px] md:items-center" :class="section.id === 'working' ? 'border-gold/15 bg-gold/[.055] ring-1 ring-inset ring-gold/20' : 'border-white/8'">
              <div class="min-w-0">
                ...
                <div class="mt-1 truncate text-xs text-muted">{{ jobSubtitle(job) }}</div>
                <div class="mt-1 flex items-center gap-2 text-[10px] text-muted"><span>Queued {{ jobDate(job.createdAt) }}</span><span v-if="job.status === 'PENDING' && job.priority" class="rounded-full bg-gold/10 px-1.5 py-0.5 font-semibold text-gold">Priority</span></div>
              </div>
              <div class="flex items-center gap-2"><span class="badge" :class="badge(job.isWorking ? job.status : jobStatus(job))">{{ jobStatus(job) }}</span><button v-if="['WAITING_FOR_ACCESS','FAILED','PARTIAL'].includes(job.status)" class="text-xs text-gold" :disabled="!features?.translation && !['SCRAPE_CHAPTERS', 'DOWNLOAD_MANGA'].includes(job.type)" @click="resumeJob(job.id)">Resume</button><button v-if="job.type === 'TRANSLATE_CHAPTER' && ['PENDING','RUNNING','CANCEL_REQUESTED'].includes(job.status)" class="text-xs text-[#efa58b]" @click="cancelJob(job.id)">Cancel</button></div>
              <div>
                <div class="mb-1 flex justify-between text-[11px] text-muted"><span>{{ job.progress }}/{{ job.total }}</span><span>{{ job.total ? Math.round(job.progress / job.total * 100) : 0 }}%</span></div>
      ...
      <p v-else-if="jobs.length" class="mt-4 text-center text-[10px] text-muted">Showing all {{ jobData?.total }} jobs in this category.</p>
    </section>
    <details class="mt-8"><summary class="btn btn-quiet cursor-pointer">Advanced source tools</summary><div class="mt-5">    <section class="surface mb-6 rounded-2xl p-6">
      <h2 class="font-serif text-2xl font-semibold">Download workers</h2>
      <p class="mt-2 text-sm text-muted">Each source downloads independently. Turn rate limiting off to skip pacing and automatic cooldown waits. Sources can still reject requests; immediate retries are limited to three. Changes apply during active downloads.</p>
    ...
    <div v-if="installedSources?.some(s => s.id === 'twkan')" class="grid gap-6 lg:grid-cols-[1.15fr_.85fr]">
      <section class="surface rounded-2xl p-6 md:p-8">
        ...
        <p class="mt-2 text-sm leading-6 text-muted">Paste either the landing page or full-directory URL. Impit is attempted first; the persistent browser is used when rendering or manual verification is needed.</p>
        <form class="mt-6" @submit.prevent="importNovel">
          <label class="mb-2 block text-xs font-semibold uppercase tracking-[.14em] text-muted">TWKAN URL</label>
          <input aria-label="TWKAN book URL" v-model="url" class="field" type="url" required />
      ...
      <section class="surface rounded-2xl p-6 md:p-8">
        <div class="flex items-center justify-between gap-3"><span class="badge" :class="browserBadge(browserState)">{{ browserState.replaceAll('_', ' ') }}</span><span class="text-[10px] uppercase tracking-[.14em] text-muted">{{ browserControl?.remoteControlEnabled ? 'Tailnet control enabled' : 'Local control only' }}</span></div>
        <h2 class="mt-5 font-serif text-2xl font-semibold">Windows browser session</h2>
        <p class="mt-2 text-sm leading-6 text-muted">Open the persistent TWKAN profile on this Windows host from localhost or your private Tailscale reader. If verification appears, this panel will warn you while the host browser waits for manual completion.</p>
