...
<template>
  <div class="mx-auto max-w-7xl px-5 py-10 md:py-14">
    <p class="mb-3 text-xs font-semibold uppercase tracking-[.24em] text-gold/80">Browse</p>
    ...
    <div class="mt-4 flex items-center justify-between gap-4"><p class="max-w-2xl text-sm leading-7 text-white/45">Browse a source directly. Nothing is added or downloaded until you choose it.</p><button class="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-white/10 transition hover:border-gold/40 hover:text-gold" :class="showAdult ? 'bg-gold/10 text-gold' : 'text-white/40'" :aria-label="showAdult ? 'Hide adult sources' : 'Show adult sources'" :title="showAdult ? 'Hide adult sources' : 'Show adult sources'" @click="showAdult = !showAdult"><svg v-if="showAdult" viewBox="0 0 24 24" class="h-5 w-5 fill-none stroke-current" stroke-width="1.8"><path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6Z"/><circle cx="12" cy="12" r="2.5"/></svg><svg v-else viewBox="0 0 24 24" class="h-5 w-5 fill-none stroke-current" stroke-width="1.8"><path d="M3 3l18 18M10.6 6.1A10.7 10.7 0 0 1 12 6c6 0 9.5 6 9.5 6a16 16 0 0 1-2.2 2.8M6.2 6.3C3.8 8 2.5 12 2.5 12s3.5 6 9.5 6c1 0 2-.2 2.9-.5M9.9 9.9a3 3 0 0 0 4.2 4.2"/></svg></button></div>
    <section class="mt-10">
      ...
      <NuxtLink v-for="source in novelSources" :key="source.id" :to="`/sources/${source.id}`" class="surface group rounded-2xl p-6 transition hover:border-gold/40">
        ...
        <p class="mt-2 text-sm text-white/45">{{ source.language }}</p>
        <div class="mt-5 flex flex-wrap gap-2"><span v-for="capability in source.capabilities" :key="capability" class="badge badge-muted">{{ capability }}</span></div>
      </NuxtLink>
      </div>
    </section>
    <section class="mt-12">
      ...
      <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        <NuxtLink v-for="source in mangaSources" :key="source.id" :to="`/sources/${source.id}`" class="surface group rounded-2xl p-6 transition hover:border-gold/40">
          <div class="flex items-start justify-between gap-4"><div class="grid h-12 w-12 place-items-center rounded-2xl bg-[#b75b65] font-serif text-xl font-bold text-white">{{ source.icon }}</div><div class="flex gap-2"><span class="badge badge-bad">Adult</span><span class="badge badge-good">Installed</span></div></div>
          <h2 class="mt-6 font-serif text-2xl font-semibold group-hover:text-gold">{{ source.name }}</h2><p class="mt-2 text-sm text-white/45">{{ source.language }}</p><div class="mt-5 flex flex-wrap gap-2"><span v-for="capability in source.capabilities" :key="capability" class="badge badge-muted">{{ capability }}</span></div>
        </NuxtLink>
        <div v-if="!mangaSources.length" class="surface rounded-2xl border-dashed p-6 text-sm text-white/30">Adult manga sources are hidden.</div>
      </div>
    </section>
  </div>
</template>
