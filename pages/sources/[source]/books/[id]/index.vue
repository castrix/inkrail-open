...
async function action(download = false) {
  ...
  try { const result: any = await $fetch(download ? `/api/downloads/${source}` : '/api/library/bookmark', { method: 'POST', body: { sourceSite: source, sourceNovelId: id } }); message.value = download ? `Queued ${result.queued} ${book.value?.mediaType === 'MANGA' ? 'pages' : 'chapters'}.` : 'Added to library.'; await refresh() }
