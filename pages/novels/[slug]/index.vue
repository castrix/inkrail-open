...
async function refreshDirectory(force = false) {
  ...
  try {
    ...
    if (state.refreshing && ++directoryAttempts < 30) directoryTimer = setTimeout(() => void refreshDirectory(), 2000)
      ...
      if (state.refreshing) directoryError.value = 'The source is taking longer than usual. Local chapters are ready to read.'
      if (!state.refreshing && !state.error) await refreshChapters()
    }
  } catch { directoryChecking.value = false; directoryError.value = 'Could not check the source. Local chapters are still available.' }
}
onMounted(() => {
  mounted = true
...
const canTranslate = (chapter: any) => features.value?.translation && chapter?.scrapeStatus === 'COMPLETED' && ['NOT_STARTED', 'FAILED', 'SOURCE_CHANGED'].includes(translationStatus(chapter))
...
async function addDailySchedule() {
  ...
  try {
    const result: any = await $fetch('/api/schedules', { method: 'POST', body: { novelId: novel.value.id, localTime: '06:00', timezone: Intl.DateTimeFormat().resolvedOptions().timeZone } })
    notice.value = `Daily sync scheduled. First run ${new Intl.DateTimeFormat('en-GB', { dateStyle: 'medium', timeStyle: 'short', timeZone: result.timezone }).format(new Date(result.nextRunAt))}.`
    await refresh()
  } catch (cause: any) {
...
<template>
  <PageError v-if="novelError" :error="novelError" @retry="refreshOverview()" />
  <div v-else-if="novel" class="mx-auto max-w-7xl px-5 pt-10 pb-[calc(8rem+env(safe-area-inset-bottom))] md:pt-14">
    ...
    <section class="grid gap-8 lg:grid-cols-[220px_1fr]">
      <div class="mx-auto aspect-[2/3] w-44 overflow-hidden rounded-2xl bg-gradient-to-br from-moss to-ink shadow-2xl lg:mx-0 lg:w-full"><BookCover :src="`/api/novels/${novel.id}/cover`" :title="novel.titleOriginal" class="h-full w-full object-cover" /></div>
      <div class="flex flex-col justify-center">
        ...
        <p v-if="novel.titleTranslated" class="mt-2 font-serif text-xl text-muted">{{ novel.titleOriginal }}</p>
        <p class="mt-4 text-sm text-muted">{{ novel.author }} · {{ novel.wordCountLabel }}</p>
        <p class="mt-6 max-w-4xl text-sm leading-7 text-muted">{{ novel.description }}</p>
    ...
    <section class="mt-12 overflow-hidden rounded-3xl border border-white/10 bg-white/[.025]">
      <button type="button" class="flex w-full items-center justify-between gap-5 p-5 text-left sm:p-7" :aria-expanded="dictionaryOpen" @click="dictionaryOpen = !dictionaryOpen">
        <div><p class="text-xs uppercase tracking-[.2em] text-gold">Spoiler-safe reference</p><h2 class="mt-2 font-serif text-2xl font-semibold sm:text-3xl">Characters & places</h2><p class="mt-2 text-xs text-muted">{{ dictionaryCounts.CHARACTER }} characters · {{ dictionaryCounts.PLACE }} places · through chapter {{ novel.readingProgress?.chapter.position || 0 }}</p></div>
        <span class="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-white/10 text-lg text-muted transition" :class="dictionaryOpen ? 'rotate-180' : ''">⌄</span>
      </button>
      <div v-if="dictionaryOpen" class="border-t border-white/8 p-5 sm:p-7">
        <p v-if="!features?.translation" class="mb-4 text-sm text-muted">Translation and character generation are disabled. In tray Settings → Edit configuration, set INKRAIL_ENABLE_TRANSLATION=true and apply configuration to enable them.</p>
        <p v-if="referencePending" role="status" class="mb-4 text-sm text-muted">Loading reference…</p>
        ...
        <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <p class="max-w-2xl text-sm leading-6 text-muted">The full downloaded-novel dictionary runs before translation to keep names and character identities consistent. This reference view only shows details through your reading progress.</p>
          <div class="flex flex-wrap gap-2"><button class="btn btn-quiet py-2" :disabled="dictionaryBusy || !features?.translation" @click="updateDictionary">{{ dictionaryBusy ? 'Queueing…' : 'Scan all downloaded chapters' }}</button><button class="btn btn-quiet py-2" :disabled="dictionaryRepairBusy || !features?.translation" @click="repairDictionaryAndTranslations">{{ dictionaryRepairBusy ? 'Queueing…' : 'Repair names & translations' }}</button></div>
        </div>
        <section class="mt-5 rounded-2xl border border-white/8 bg-black/10 p-4">
          <div class="flex items-center justify-between gap-3"><div><h3 class="font-serif text-xl font-semibold">Main character art</h3><p class="mt-1 text-xs text-muted">Candidates and skins unlock only through chapter {{ novel.readingProgress?.chapter.position || 0 }}.</p></div><button class="btn btn-quiet shrink-0 px-3 py-2 text-xs" :disabled="Boolean(visualBusy)" @click="refreshCharacterVisuals">{{ visualBusy === 'refresh' ? 'Scoring…' : 'Refresh' }}</button></div>
        ...
        <p v-else class="mt-5 rounded-2xl border border-dashed border-white/10 px-5 py-8 text-center text-sm text-muted">{{ dictionarySearch ? 'No dictionary entries match this search.' : 'No spoiler-safe entries are known at this reading position yet.' }}</p>
      </div>
    </section>
      ...
      <div v-if="activeSkinProfile" class="fixed inset-0 z-[90] flex items-end justify-center bg-black/75 p-0 backdrop-blur-sm sm:items-center sm:p-6" @click.self="skinProfileId = ''">
        <section class="flex max-h-[92dvh] w-full max-w-3xl flex-col overflow-hidden rounded-t-3xl border border-white/10 bg-[#171915] shadow-2xl sm:rounded-3xl">
          <header class="flex items-start justify-between gap-4 border-b border-white/8 p-4 sm:p-6">
            <div class="min-w-0"><p class="text-[10px] uppercase tracking-[.2em] text-gold">Character skins</p><h2 class="mt-1 truncate font-serif text-2xl font-semibold">{{ activeSkinProfile.name }}</h2><p class="mt-1 text-xs text-muted">Generated skins are enabled automatically. Future details unlock with your reading progress.</p></div>
            <button class="grid h-9 w-9 shrink-0 place-items-center rounded-full border border-white/10 text-muted" aria-label="Close skin timeline" @click="skinProfileId = ''">×</button>
          </header>
          <div class="flex flex-wrap gap-2 border-b border-white/8 p-3 sm:px-6">
            <button class="btn btn-quiet px-3 py-2 text-xs" :disabled="Boolean(visualBusy) || !features?.translation || ['QUEUED','RUNNING'].includes(activeSkinProfile.skinDetectionStatus)" @click="detectSkins(activeSkinProfile.entryId)">{{ ['QUEUED','RUNNING'].includes(activeSkinProfile.skinDetectionStatus) ? 'Detecting…' : activeSkinProfile.skinDetectedAt ? 'Rescan source' : 'Detect skins' }}</button>
            <button class="btn btn-primary px-3 py-2 text-xs" :disabled="Boolean(visualBusy) || !features?.translation || !selectedSkinIds.length" @click="generateSkins('selected')">Generate selected ({{ selectedSkinIds.length }})</button>
            <button class="btn btn-quiet px-3 py-2 text-xs" :disabled="Boolean(visualBusy) || !features?.translation || !features?.translation" @click="generateSkins('unlocked')">Generate unlocked</button>
            <button class="btn btn-quiet px-3 py-2 text-xs" :disabled="Boolean(visualBusy) || !features?.translation || !features?.translation" @click="generateSkins('all')">Generate all</button>
          </div>
          ...
          <div class="min-h-0 flex-1 space-y-3 overflow-y-auto p-4 sm:p-6">
            <article v-for="skin in activeSkinProfile.skins" :key="skin.id" class="relative overflow-hidden rounded-2xl border p-3" :class="skin.locked ? 'border-white/6 bg-white/[.02]' : skin.status === 'DISMISSED' ? 'border-white/5 opacity-75' : 'border-white/10 bg-white/[.025]'">
              <div v-if="skin.locked" class="flex min-h-24 items-center gap-3">
                ...
                <div><h3 class="font-semibold text-white/60">Locked skin</h3><p class="mt-1 text-xs text-muted">Unlocks at chapter {{ skin.introducedAtPosition }}</p><span class="badge badge-muted mt-2 text-[9px]">{{ skin.status.replaceAll('_', ' ') }}</span></div>
              </div>
              <div v-else class="flex items-start gap-3">
                ...
                <div class="h-24 w-20 shrink-0 overflow-hidden rounded-xl bg-white/5"><img v-if="skin.imageUrl" :src="skin.imageUrl" :alt="skin.name" loading="lazy" decoding="async" class="h-full w-full object-cover" /><span v-else class="grid h-full place-items-center text-2xl text-muted">◇</span></div>
                <div class="min-w-0 flex-1"><div class="flex flex-wrap items-center gap-2"><h3 class="font-semibold">{{ skin.name }}</h3><span class="badge badge-muted text-[9px]">{{ skin.status.replaceAll('_', ' ') }}</span></div><p class="mt-1 text-[10px] uppercase tracking-[.12em] text-muted">Chapter {{ skin.introducedAtPosition }} · {{ skin.changeType?.replaceAll('_', ' ') }}</p><p class="mt-2 text-sm leading-5 text-muted">{{ skin.description || 'No description yet.' }}</p><div class="mt-3 flex flex-wrap gap-3"><button v-if="!['QUEUED','GENERATING','DISMISSED'].includes(skin.status)" class="text-xs text-[#efa58b] hover:text-white" :disabled="Boolean(visualBusy)" @click="setSkinDismissed(skin.id, true)">Remove skin</button><button v-if="skin.status === 'DISMISSED'" class="text-xs text-muted hover:text-white" :disabled="Boolean(visualBusy)" @click="setSkinDismissed(skin.id, false)">Restore</button><a v-if="skin.fullBodyImageUrl" :href="skin.fullBodyImageUrl" target="_blank" class="text-xs text-gold">Full body</a></div></div>
              </div>
            </article>
            <p v-if="!activeSkinProfile.skins.length" class="rounded-2xl border border-dashed border-white/10 px-5 py-10 text-center text-sm text-muted">No skins yet. Detect visual changes from the downloaded Chinese source.</p>
          </div>
        </section>
      </div>
    </Teleport>
    ...
    <section id="chapters" class="mt-14 scroll-mt-24">
      <div class="mb-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div><p class="text-xs uppercase tracking-[.2em] text-muted">Directory</p><h2 class="mt-2 font-serif text-3xl font-semibold">{{ chapterResult?.directoryCount ?? novel.chapterCount }} entries</h2><p class="mt-2 text-xs text-muted">{{ chapterResult?.localCount ?? novel.chapterCount }} indexed locally</p></div>
        <div class="flex w-full flex-col gap-3 sm:w-auto sm:items-end"><div class="grid w-full grid-cols-2 gap-2 sm:flex sm:w-auto sm:flex-wrap sm:justify-end"><button class="btn btn-quiet w-full px-3 text-sm sm:w-auto" :disabled="syncing" @click="syncChapters">{{ syncing ? 'Syncing…' : 'Sync & download new' }}</button><details v-if="features?.translation" ref="translateMenu" class="group relative"><summary class="btn btn-primary flex w-full cursor-pointer list-none items-center justify-center gap-2 px-3 text-sm sm:w-auto" :class="queuing ? 'pointer-events-none opacity-75' : ''"><span>{{ queuing ? 'Queueing…' : 'Translate' }}</span><span class="text-[10px] transition-transform group-open:rotate-180">▼</span></summary><div class="absolute right-0 z-30 mt-2 w-64 overflow-hidden rounded-xl border border-white/10 bg-[#20231e] p-1.5 shadow-2xl"><button class="w-full rounded-lg px-3 py-2.5 text-left text-sm hover:bg-white/[.06]" @click="queueTranslations(undefined, 'all')"><span class="block font-semibold">All untranslated</span><span class="mt-0.5 block text-xs text-muted">Every downloaded chapter</span></button><button class="w-full rounded-lg px-3 py-2.5 text-left text-sm hover:bg-white/[.06] disabled:cursor-not-allowed disabled:opacity-75" :disabled="!novel.readingProgress" @click="queueTranslations(undefined, 'recent')"><span class="block font-semibold">Recent only</span><span class="mt-0.5 block text-xs text-muted">{{ novel.readingProgress ? `Chapter ${novel.readingProgress.chapter.position} onward` : 'Start reading to set the boundary' }}</span></button></div></details><button class="btn btn-quiet col-span-2 w-full px-3 text-sm sm:col-auto sm:w-auto" :disabled="downloading" @click="downloadChapters()">{{ downloading ? 'Indexing…' : 'Download all' }}</button></div><div class="flex self-end rounded-xl border border-white/10 p-1 text-xs">
          <button v-for="option in ['all','ready','untranslated']" :key="option" class="rounded-lg px-3 py-2 capitalize" :class="filter === option ? 'bg-white/10 text-white' : 'text-muted'" @click="filter = option as any">{{ option }}</button>
        </div></div>
      </div>
      <div class="mb-4 flex items-center gap-2 rounded-xl border border-white/10 bg-white/[.025] px-3 py-2.5 focus-within:border-gold/40">
        ...
        <input v-model="chapterSearch" type="search" class="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-muted" placeholder="Search chapter number or title…" aria-label="Search chapters" />
        <span v-if="chapterSearch" class="shrink-0 text-[10px] text-muted">{{ chapterResult?.total ?? 0 }} found</span>
        <button v-if="chapterSearch" class="shrink-0 px-1 text-xs text-muted hover:text-white" aria-label="Clear chapter search" @click="chapterSearch = ''">Clear</button>
      </div>
      <div class="sticky top-[99px] z-20 mb-3 rounded-lg border border-white/10 bg-[#171915]/95 p-1.5 shadow-xl backdrop-blur-xl sm:hidden">
        <div class="flex min-w-0 items-center gap-1">
          <span class="min-w-0 flex-1 truncate px-1 text-[10px] text-muted">{{ selected.length ? `${selected.length} selected · ${selectedTranslatable.length} ready` : 'Shift-click for a range' }}</span>
          <button class="rounded-md border border-white/10 px-2 py-1.5 text-[10px] font-semibold text-white/65 disabled:opacity-75" :disabled="chapterPending || Boolean(chapterError) || !visibleSelectable.length" @click="toggleVisibleSelection">{{ allVisibleSelected ? 'None' : 'Visible' }}</button>
          ...
          <button v-if="selected.length" class="px-1.5 py-1 text-[10px] text-muted" @click="clearSelection">Clear</button>
        </div>
        <div v-if="selected.length" class="mt-1 grid grid-cols-2 gap-1">
          <button class="rounded-md bg-gold px-2 py-1.5 text-[11px] font-semibold text-ink disabled:opacity-75" :disabled="queuing || !selectedTranslatable.length" @click="queueTranslations(selectedTranslatable)">Translate ({{ selectedTranslatable.length }})</button>
          <button class="rounded-md border border-white/10 bg-white/[.04] px-2 py-1.5 text-[11px] font-semibold disabled:opacity-75" :disabled="queuing || !selectedCancelable.length" @click="cancelTranslations(selectedCancelable)">Cancel ({{ selectedCancelable.length }})</button>
        </div>
      </div>
      <div class="sticky top-[77px] z-20 mb-4 hidden flex-wrap items-center gap-3 rounded-2xl border border-white/10 bg-[#171915]/95 p-3 shadow-xl backdrop-blur-xl sm:flex">
        ...
        <div class="flex min-w-[180px] flex-1 items-center gap-3 text-xs text-muted">
          ...
          <button v-if="selected.length" class="shrink-0 text-muted hover:text-white" @click="clearSelection">Clear</button>
        </div>
      ...
      <p v-if="directoryChecking" role="status" class="mb-3 text-xs text-muted">Checking TWKAN for new chapters…</p>
      <p v-if="directoryError" class="mb-3 text-xs text-muted">{{ directoryError }} <button class="text-gold underline" @click="refreshDirectory(true)">Retry source check</button></p>
      ...
      <p v-if="chapterPending" role="status" class="mb-3 text-sm text-muted">Loading chapters…</p>
      <p v-if="!chapters.length && !chapterPending && !chapterError" class="py-10 text-center text-muted">{{ chapterSearch || filter !== 'all' ? 'No matching chapters.' : 'No chapters indexed yet.' }}</p>
      <div class="surface overflow-hidden rounded-2xl">
        <div v-for="chapter in chapters" data-chapter-row :key="chapter.sourceChapterId" class="grid grid-cols-[24px_48px_minmax(0,1fr)_auto] items-center gap-3 border-b border-white/8 px-4 py-3.5 transition-colors last:border-0" :class="selectedSet.has(chapter.id) ? 'bg-gold/[.08] ring-1 ring-inset ring-gold/20' : 'hover:bg-white/[.025]'">
          <input type="checkbox" class="h-4 w-4 cursor-pointer accent-[#d4a85f] disabled:cursor-not-allowed disabled:opacity-75" :disabled="chapterPending || Boolean(chapterError) || !canSelect(chapter)" :checked="selectedSet.has(chapter.id)" :aria-label="`Select ${chapter.titleOriginal}`" @click="toggleChapter(chapter.id, $event)" />
          <span class="font-mono text-xs text-muted">{{ String(chapter.position).padStart(3, '0') }}</span>
          <NuxtLink v-if="chapter.scrapeStatus === 'COMPLETED'" :to="`/novels/${novel.slug}/chapters/${chapter.id}`" class="group min-w-0"><div class="truncate text-sm font-medium group-hover:text-gold">{{ chapter.titleTranslated || chapter.titleOriginal }}</div><div v-if="chapter.titleTranslated" class="mt-1 truncate text-xs text-muted">{{ chapter.titleOriginal }}</div></NuxtLink>
          <button v-else class="min-w-0 text-left" :disabled="openingChapter === chapter.sourceChapterId" @click="openChapter(chapter)"><div class="truncate text-sm text-muted hover:text-gold">{{ openingChapter === chapter.sourceChapterId ? 'Opening and downloading…' : chapter.titleOriginal }}</div></button>
          <div class="flex items-center gap-2">
            ...
            <span v-else-if="['PENDING','DOWNLOADING'].includes(chapter.scrapeStatus)" class="text-xs text-muted">{{ chapter.scrapeStatus === 'DOWNLOADING' ? 'Downloading…' : 'Queued' }}</span>
            <button v-else-if="canTranslate(chapter)" class="text-xs text-gold hover:text-white" :disabled="queuing || !features?.translation" @click="queueTranslations([chapter.id])">Translate</button>
            <button v-else-if="canCancel(chapter)" class="text-xs text-[#efa58b] hover:text-white" :disabled="queuing || !features?.translation" @click="cancelTranslations([chapter.id])">Cancel</button>
