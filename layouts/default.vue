...
function closeNavigation() { mobileMenuOpen.value = false; document.querySelectorAll('header details[open]').forEach(el => el.removeAttribute('open')) }
watch(() => route.fullPath, () => { if (import.meta.client) closeNavigation() })
...
<template>
  <div class="min-h-screen" @keydown.esc="closeNavigation">
    ...
    <header v-if="route.path !== '/login' && !isReader" class="sticky top-0 z-40 border-b border-white/10 bg-[#101210]/90 backdrop-blur-xl">
      <div class="mx-auto flex max-w-7xl items-center justify-between px-5 py-4">
        <NuxtLink to="/library" class="flex items-center gap-3">
          ...
          <div>
            ...
            <div class="text-[10px] uppercase tracking-[.2em] text-muted">your reading room</div>
          </div>
        </NuxtLink>
        <nav class="hidden items-center gap-2 text-sm xl:flex">
          ...
          <details class="relative"><summary class="btn btn-quiet cursor-pointer">More</summary><div class="absolute right-0 top-full z-50 grid w-52 gap-1 rounded-xl border border-white/10 bg-[#181b18] p-2"><NuxtLink class="btn btn-quiet" to="/extensions">Extensions</NuxtLink>
          ...
          <button class="btn btn-quiet" @click="logout">Lock</button></div></details>
        </nav>
        <button type="button" class="grid h-11 w-11 place-items-center rounded-xl border border-white/10 bg-white/[.04] text-white xl:hidden" :aria-expanded="mobileMenuOpen" aria-controls="mobile-navigation" :aria-label="mobileMenuOpen ? 'Close navigation menu' : 'Open navigation menu'" @click="mobileMenuOpen = !mobileMenuOpen">
      ...
      <nav v-if="mobileMenuOpen" id="mobile-navigation" class="border-t border-white/10 px-5 py-3 xl:hidden" aria-label="Mobile navigation">
    ...
    <component :is="isReader ? 'div' : 'main'" id="main-content" tabindex="-1"><slot /></component>
    <JobQueueToolbar v-if="route.path !== '/login' && !isReader" />
  </div>
</template>
