...
test('wizard creates credentials and preserves private unrelated settings on later saves', () => {
  ...
  try {
    ...
    assert.equal(env.SCRAPER_DNS_ENABLED, 'true')
    assert.equal(env.SCRAPER_DNS_SERVERS, '1.1.1.1,1.0.0.1')
    const secret = env.OWNER_SESSION_SECRET
    result = run('save', { port: '4100', host: '0.0.0.0', password: 'another-password', dnsEnabled: 'false', dnsServers: '8.8.8.8' })
    assert.equal(result.status, 0, result.stderr)
    env = parseEnv(readFileSync(join(dir, '.env'), 'utf8'))
    assert.equal(env.OWNER_SESSION_SECRET, secret)
    assert.equal(env.SCRAPER_DNS_ENABLED, 'false')
    assert.equal(env.SCRAPER_DNS_SERVERS, '8.8.8.8')
    assert.equal(JSON.parse(run('read').stdout).dnsEnabled, 'false')
    assert.equal(env.CUSTOM_FLAG, 'keep-me')
    assert.equal(env.DATABASE_URL, 'file:../data/database/inkrail.db')
    assert.notEqual(run('save', { port: '4100', host: '127.0.0.1', password: 'another-password', dnsServers: 'bad\nvalue' }).status, 0)
