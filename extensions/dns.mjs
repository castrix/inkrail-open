...
async function resolveHost(state, host) {
  ...
  for (const server of state.resolverServers) {
    try {
      ...
      state.cache.set(key, { addresses, expiresAt: Date.now() + ttl * 1e3 });
      return addresses;
    } catch (error) {
      lastError = error;
    }
...
function openSocket(addresses, port) {
...
async function startProxy() {
  ...
  server.on("connect", (request, client, head) => {
    ...
    void resolveHost(state, target.host).then((addresses) => openSocket(addresses, target.port)).then((upstream) => {
      ...
      client.end(`HTTP/1.1 502 Bad Gateway\r
...
async function scraperProxyUrl() {
  if (process.env.SCRAPER_DNS_ENABLED === "false") return null;
  globalDns.inkrailScraperDns ||= startProxy().catch((error) => {
    globalDns.inkrailScraperDns = void 0;
    throw error;
  });
  return (await globalDns.inkrailScraperDns).url;
}
async function scraperDnsStatus() {
